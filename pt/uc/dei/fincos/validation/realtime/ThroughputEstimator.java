package pt.uc.dei.fincos.validation.realtime;

import java.util.LinkedList;

import pt.uc.dei.fincos.basic.Globals;
import pt.uc.dei.fincos.validation.PerformanceMonitor.WindowEvaluationModel;
...
public class ThroughputEstimator {
	...
	public double computeCurrentThroughput() {		
		...
		synchronized (eventsTimestampsBuffer) {
			...
			for (int i = 0; i < eventsTimestampsBuffer.size() && now-eventsTimestampsBuffer.get(i)/rtFactor > windowSizeInMilliSeconds; ) {
					eventsTimestampsBuffer.remove(i);
			}						
			
			throughput = modFactor*eventsTimestampsBuffer.size()*1000.0/windowSizeInMilliSeconds;			
		}
