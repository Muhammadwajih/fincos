...
public class EsperInterface extends CEPEngineInterface {
	 ...
	 * @param configuredStreams			The list of streams in the test configuration file (required to create POJO events) 
	 */
		...
		if(this.status.getStep() == Step.CONNECTED) {
			...
			for (Entry<String, String> query : this.queryNamesAndTexts.entrySet()) {
				...
				if(sinksList != null && !sinksList.isEmpty()) {
					...
					System.err.println("WARNING: Query \"" + query.getKey() + "\" has no corresponding Sink.");
				}				
			}
	...
	private void sendMapEvent(String event) {
		...
		try {									
			if(eventSchema != null) {		
				for (Entry <String, String> field: eventSchema.entrySet()) {						
					if(field.getValue().equals("int"))
						mapEvent.put((String)field.getKey(), Integer.parseInt(eventPayload[i]));
					else if(field.getValue().equals("long"))
						mapEvent.put((String)field.getKey(), Long.parseLong(eventPayload[i]));
					else if(field.getValue().equals("string"))
						mapEvent.put((String)field.getKey(), eventPayload[i]);
					else if(field.getValue().equals("double"))
						mapEvent.put((String)field.getKey(), Double.parseDouble(eventPayload[i]));
					else if(field.getValue().equals("float"))
						mapEvent.put((String)field.getKey(), Float.parseFloat(eventPayload[i]));
			...
			System.err.println("ERROR: Number of fields in event \"" + event + "\" ("+ (eventPayload.length-1)+
		...
		try {									
			if(eventSchema != null) {		
				for (Entry <String, String> field: eventSchema.entrySet()) {											
					...
					{
						mapEvent.put((String)field.getKey(), event.getTimestamp());
					}	
					else {
						mapEvent.put((String)field.getKey(), event.getAttributeValue(i));	
					}
			...
			for (int i = 0; i < eventFields.length; i++) {
					...
					i == eventFields.length-1) 
				{
					...
					if(f.getType() == int.class)
						...
						f.set(pojoEvent, (String) event.getAttributeValue(i));
					else if(f.getType() == double.class)
