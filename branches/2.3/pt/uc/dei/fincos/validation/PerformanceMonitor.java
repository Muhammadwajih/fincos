...
public class PerformanceMonitor extends JFrame{			
	...
	private ArrayList<ServerSocket> ssList; // one Server Socket for each port
	ArrayList<Receiver> workers; // worker threads for receiving events from server Sockets

	private ConfigurationParser config = new ConfigurationParser();
	private ArrayList<DriverConfig> drivers; // List of drivers configured to send events to this Perfmon instance
	private ArrayList<SinkConfig> sinks; // List of sinks configured to send events to this Perfmon instance
	 ...
	 *  TIME_BASED: At specified intervals (in this case every time GUI is refreshed, 1 per second) 
	 *  TUPLE_BASED: Every time a new event arrives
	 */	
	public enum WindowEvaluationModel{TIME_BASED, TUPLE_BASED};
	...
	//====================== End of Real time performance monitoring variables =====================
	...
	//================== End of Offline Performance Measuring (Sink logs) variables =================	
	...
	private void loadForSinkLogFile(String inputSinkLogFiles[], boolean saveToFile,	String outputPerfMonLogFile, 
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Could not process log files (" +
	...
	private void loadStatsPanel(TreeMap<String, HashSet<Stream>> serversStreams) {
		...
		avgThroughput=minThroughput=maxThroughput=lastThroughput=avgRT=minRT=maxRT=lastRT=stdevRT=0;
	    ...
	    private void saveToFileCheckItemStateChanged(java.awt.event.ItemEvent evt) {
	        ...
	        if(saveToFileCheck.isSelected())
	        	saveToFileField.setText(Globals.APP_PATH+"log"+
	        							File.separator+"FINCoS_PerfmonLog_"+
